...
static float analog2temp(int raw, uint8_t e) {
  ...
  return ((raw * ((3.3 * 100.0) / 1024.0) / OVERSAMPLENR) * EXTRUDER_TEMP_GAIN0) + EXTRUDER_TEMP_OFFSET0;
  /*
  switch (extr_type)
  {
	case 0:
	...
	break;
	case 1:
		return ((raw * ((3.3 * 100.0) / 1024.0) / OVERSAMPLENR) * EXTRUDER_TEMP_GAIN1) + EXTRUDER_TEMP_OFFSET1;
	break;
	default:
	return ((raw * ((3.3 * 100.0) / 1024.0) / OVERSAMPLENR) * EXTRUDER_TEMP_GAIN0) + EXTRUDER_TEMP_OFFSET0;
...
static float analog2tempBed(int raw) {
    ...
    return ((raw * ((3.3 * 100.0) / 1024.0) / OVERSAMPLENR) * BED_TEMP_GAIN) + BED_TEMP_OFFSET;
  #else
    return 0;
  #endif
}
...
void tp_init()
{
  ...
  for(int e = 0; e < EXTRUDERS; e++) {
	...
	SET_OUTPUT(FAN_PIN2);
    #ifdef FAST_PWM_FAN
    (DualExtMode)?setPwmFrequency(FAN_PIN2, 1):setPwmFrequency(FAN_PIN, 1); 
  ...
  HAL_timer_start (TEMP_TIMER_NUM, 2000);   
...
HAL_TEMP_TIMER_ISR
{
  ...
  if(pwm_count == 0){
    ...
    soft_pwm_fan = fanSpeedSoftPwm / 2;
	if (DualExtMode){
		if(soft_pwm_fan > 0) WRITE(FAN_PIN2,1); else WRITE(FAN_PIN2,0);
	} else {
  ...
  #ifdef FAN_SOFT_PWM
  if (DualExtMode){
	  if(soft_pwm_fan < pwm_count) WRITE(FAN_PIN2,0);
  } else {
