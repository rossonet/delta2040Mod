...
int8_t SdBaseFile::lsPrintNext( uint8_t flags, uint8_t indent) {
  ...
  for (uint8_t i = 0; i < indent; i++) (wifi_off)?MYSERIAL.write(' '):WIFI.write(' ');

  // print name
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ')continue;
    if (i == 8) {
      (wifi_off)?MYSERIAL.write('.'):WIFI.write('.');
      w++;
    }
    (wifi_off)?MYSERIAL.write(dir.name[i]):WIFI.write(dir.name[i]);
    w++;
  }
  ...
  if (flags & (LS_DATE | LS_SIZE)) {
    while (w++ < 14) (wifi_off)?MYSERIAL.write(' '):WIFI.write(' ');
  }
  ...
  if (!DIR_IS_SUBDIR(&dir) && (flags & LS_SIZE)) {
	...
	WIFI.write(' ');
    WIFI.print(dir.fileSize);	
	}
  }
  (wifi_off)?MYSERIAL.println():WIFI.println();
...
bool SdBaseFile::printName() {
  ...
  (wifi_off)?MYSERIAL.print(name):WIFI.print(name);
  return true;
}
//------------------------------------------------------------------------------
