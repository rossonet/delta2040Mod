...
bool abort_on_endstop_hit = true;   //default enabled
#endif
...
if(step_rate < (F_CPU/500000)) step_rate = (F_CPU/500000);
  ...
  if(step_rate >= (8*256)){ // higher step rate
    ...
    timer = 1(unsigned short)pgm_read_word_near(table_address);
    timer -= (((unsigned short)pgm_read_word_near(table_address+2) * (unsigned char)(step_rate & 0x0007))>>3);
  }
#elif defined(ARDUINO_ARCH_SAM)
if(step_rate < (F_CPU/2625000)) step_rate = (F_CPU/2625000);
//  timer = HAL_TIMER_RATE / step_rate;


 step_rate -= (F_CPU/2625000); // Correct for minimal speed
  if(step_rate >= (8*256)){ // higher step rate
    unsigned char tmp_step_rate = (step_rate & 0x00ff);
    ...
    timer = speed_lookuptable_slow[(((step_rate)>>1) & 0xfffc)>>2][0];
    timer -= ((speed_lookuptable_slow[(((step_rate)>>1) & 0xfffc)>>2][1] * (unsigned char)(step_rate & 0x0007))>>3);
  }
...
#if defined(SENS_BED) && SENS_BED > -1
SET_INPUT(SENS_BED);
PULLUP(SENS_BED,HIGH);
#endif
#if defined(CURR_CONTROL_DRIVER) && CURR_CONTROL_DRIVER > -1
	SET_OUTPUT(CURR_CONTROL_DRIVER);			// init current drivers when init stepper
  ...
  //Initialize Step Pins
    ...
    disable_e2();
  #endif
	SET_OUTPUT(STEPPER_RESET);
	WRITE(STEPPER_RESET,HIGH);
  ...
  HAL_timer_start (STEP_TIMER_NUM, 26250);
